...
class Dense():
    ...
    def upgrade(self, delta):
        ...
        self.W -= self.config.lr * dW + self.config.beta * self.W

        db = np.mean(delta, axis=0, keepdims=True)
        self.b -= self.config.lr * db
...
class Model():
    ...
    def loss(self, y, y_pred):
        ...
        loss += np.sum(self.layer_output.W ** 2) * self.config.beta
        for layer in self.layers:
            loss += np.sum(layer.W ** 2) * self.config.beta
        return loss
